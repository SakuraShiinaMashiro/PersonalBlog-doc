...
* 用户登录完之后，原有的登录按钮，也就是人物头像图标，显示用户圆角头像

  1. 如果是游客，还需在圆角头像右下侧贴上对应的第三方登录的图标，也是圆角
  2. 如果是博主，无需显示对应的第三方登录的图标

* 用户点击自己的头像，在头像的下方弹出一个退出登录的小卡片，卡片布局如下
...
4. 点击第三方登录图标时，以新窗口弹出授权页，当前页面不跳转。
5. 第三方授权成功后，关闭弹出的授权窗口，主页面继续保持并完成登录态刷新。
...
```mermaid
flowchart TD
    A["已登录用户自己的圆角头像(写文章右侧，与登录人物图标同位置)"] --> B[弹出登出小卡片]
    B --> C[点击登出]
    C --> F[调用退出接口]
    F --> G[清理本地 Token]
    G --> H[提示退出成功]
