...
- `AuthController#ownerLogin(OwnerLoginDTO dto)`：账号密码校验。
- `AuthController#sendEmailCode(EmailCodeDTO dto)`：发送邮箱验证码。
- `AuthController#verifyEmailCode(EmailVerifyDTO dto)`：验证码校验并签发 Token。
...
3. 账号密码通过 -> 跳转发送验证码页面，自动显示博主的邮箱，博主只需点击发送验证码，并填写邮箱验证码，点击确认 -> 校验成功下发 Token。
...
- [x] **API 封装**: 
    - `auth.ts`: 密钥校验、博主登录、发送验证码、验证码校验、登出、OAuth 链接。
...
- [x] **`AppLoginDialog.vue`**: 
    ...
    - 实现三步切换流：1. OAuth/导入密钥入口 -> 2. 账号密码输入 -> 3. 邮箱验证码校验。
